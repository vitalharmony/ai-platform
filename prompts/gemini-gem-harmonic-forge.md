...
Service lifecycle: `mise run restart`/`check`/`bump`/`commit` (mise + process-compose, adopted 2026-07-13 per harmonic-forge ADR-001, replacing the retired hrse_manager.py) is the only correct method for restarts, commits, and version bumps. Do not suggest direct uvicorn, npm, or process commands as substitutes.
